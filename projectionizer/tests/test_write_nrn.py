...
def test_create_synapse_data():
    ...
    assert_allclose(write_nrn.create_synapse_data(df, SYNAPSE_PARAMS), correct_result)
...
def test_write_synapses():
    ...
    with setup_tempdir('test_utils') as folder:
        ...
        write_nrn.write_synapses(path, df.groupby('tgid'), SYNAPSE_PARAMS)
        ...
        efferent_path = os.path.join(folder, 'projectionizer_test_write_synapses_efferent.h5')
...
def _write_synapses(synapse_data={}, synapse_params={}):
    ...
    with setup_tempdir('test_utils') as folder:
        path = os.path.join(folder, 'projectionizer_test_write_synapses.h5')
        write_nrn.write_synapses(path, df.groupby('tgid'), _synapse_params)
