...
def test_load_morphology():
    ...
    morph = test_module.load_morphology(morph_name='morph',
                                        morph_path=TEST_DATA_DIR,
                                        morph_type='swc')
...
def test_compute_afferent_section_pos(caplog):
    ...
    expected = [0.15599452, 0.05808361, 0.89705235, 0.14286682, 0.6508885,
                0.05641158, 0.97411096, 0.34576055, 0.9922116, 0.10141408, ]
...
def test_get_morphs_for_nodes():
    ...
    pd.testing.assert_frame_equal(
            pd.DataFrame(['morph'], columns=['morph'], index=[1]),
            morphs
    )
