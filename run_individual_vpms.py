...
def generate_vpm(cfg_file, xml_file):
    ...
    cd %s
    srun --account=proj1 -t 3-00:00:00 --partition=prod --job-name="prjctnzr" bash -c ". ~/venv/bin/activate && python $HOME/src/Projectionizer-git/projection_runner_tcs2f.py %s %s %s 2>&1 | tee prjctnzr.log"
    ...
    ret = os.system("""
    srun --account=proj1 --mem=28000 -n16 -t 3-00:00:00 --partition=prod --job-name="prune_runner_ps" bash -c ". ~/venv/bin/activate && cd %s && python $HOME/src/Projectionizer-git/projection_prune_runner_ps.py %s 2>&1 | tee prune_runner_ps.log"
    """ % (out_dir, cfg_file))
    ...
    if ret!=0:
        print "transpose and splitter failed."
        sys.exit(1)
    ...
    FLATIndex.buildIndex(%s,"synapse","SYNAPSE",0,"Mosaic")
